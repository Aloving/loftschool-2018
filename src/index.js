...
function findAllPSiblings(where) {
    return Array.prototype.reduce.call(where.childNodes, (result, current) => {
        if (current.nextSibling && current.nextSibling.tagName === 'P') {
            result.push(current);
        }
...
function findError(where) {
    ...
    for (var child of where.children) {
        if (child.children > 0) {
            result = result.concat(findError(child));
        }
